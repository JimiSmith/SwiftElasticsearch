extension Request {
     ...
     */
    public static func snapshotRestore(repository: String, snapshot: String, method: HttpMethod = .POST, body: ElasticsearchBody?) -> Request {
        assert(method == .POST)
     ...
     * - parameter body: The body to be sent with the request
     */
    public static func snapshotRestore(repository: String, snapshot: String, method: HttpMethod = .POST, body: [String : Any]?) -> Request {
        assert(method == .POST)
        let url = "/_snapshot/\(repository)/\(snapshot)/_restore"
